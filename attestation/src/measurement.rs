...
use crate::{
    ...
    kdf::{attest_secret_key, tsm_cdi, tvm_attest_cdi, tvm_attest_cdi_id},
    Error, Result,
};
...
trait CdiBuilder<'a> {
    ...
    fn build_next_cdi<D: Digest, H: HmacImpl<D>>(
        &mut self,
        id: u64,
        measurements: &ArrayVec<MeasurementRegister<D>, MSMT_REGISTERS>,
    ) -> Result<()>;

    // Get the next layer CDI.
...
impl<'a, T> Cdi<T> {
    fn new<D: Digest, H: HmacImpl<D>>(cdi: &'a [u8]) -> Result<Self> {
        ...
        tsm_cdi::<D, H>(cdi, &mut current_cdi)?;
        Ok(Cdi {
            current_cdi,
            next_cdi: None,
            _pd_t: PhantomData,
...
impl<'a> CdiBuilder<'a> for Cdi<AttestationCdi> {
    fn build_next_cdi<D: Digest, H: HmacImpl<D>>(
        &mut self,
        id: u64,
        _measurements: &ArrayVec<MeasurementRegister<D>, MSMT_REGISTERS>,
    ) -> Result<()> {
        let mut cdi_attest = [0u8; CDI_LEN];
        tvm_attest_cdi::<D, H>(&self.current_cdi, id, &mut cdi_attest)?;
...
pub struct AttestationManager<D: Digest, H: HmacImpl<D> = hmac::Hmac<D>> {
    ...
    attest_cdi: RwLock<Cdi<AttestationCdi>>,
...
impl<'a, D: Digest, H: HmacImpl<D>> AttestationManager<D, H> {
    ...
    pub fn new(cdi: &'a [u8], vm_id: u64, hash_algorithm: ObjectIdentifier) -> Result<Self> {
        ...
        Ok(AttestationManager {
            ...
            attest_cdi: RwLock::new(Cdi::new::<D, H>(cdi)?),
            vm_id,
            _pd: PhantomData,
        })
    }
    ...
    pub fn finalize(&self) -> Result<()> {
        ...
        let mut cdi = self.attest_cdi.write();
        let measurements = self.measurements.read();
        cdi.build_next_cdi::<D, H>(self.vm_id, &measurements)
    }
    ...
    pub fn attestation_key_pair(&self) -> Result<Keypair> {
        let mut secret_bytes = [0u8; SECRET_KEY_LENGTH];
        attest_secret_key::<D, H>(&self.attest_cdi.read().next_cdi()?, &mut secret_bytes)?;
    ...
    /// Encode the Attestation CDI ID into a UTF-8 slice.
    pub fn attestation_cdi_id(&self, public_key: &[u8], output: &mut [u8]) -> Result<()> {
        let mut cdi_id_bytes = [0u8; CDI_ID_LEN];
        tvm_attest_cdi_id::<D, H>(public_key, &mut cdi_id_bytes)?;
        hex::encode_to_slice(cdi_id_bytes, output).map_err(Error::InvalidCdiId)
    }
}
