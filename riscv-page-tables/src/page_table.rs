...
pub enum Error {
    ...
    PageNotShareable,
    /// Invalid address, should be page aligned
    InvalidPageAddress(u64),
}
...
impl<T: FirstStagePagingMode> FirstStagePageTable<T> {
    ...
    pub fn map_range(
        ...
        if !page_size.is_aligned(addr.bits()) {
            return Err(Error::InvalidPageAddress(addr.bits()));
        }

        let iterator = match addr.iter_from_with_size(page_size) {
            Some(i) => i,
            None => return Err(Error::OutOfMapRange),
        };
...
impl<'a, T: FirstStagePagingMode> FirstStageMapper<'a, T> {
    ...
    pub unsafe fn map_addr(
        ...
        if !page_size.is_aligned(vaddr.bits()) {
            return Err(PageTableError::InvalidPageAddress(vaddr.bits()));
        }
        if !page_size.is_aligned(paddr.bits()) {
            return Err(PageTableError::InvalidPageAddress(paddr.bits()));
        }

        if let Some(end_vaddr) = self
            .vaddr
            .checked_add_pages_with_size(self.num_pages, page_size)
        {
        ...
        inner.map_leaf(vaddr, paddr, page_size, pte_perms)
    }
}
